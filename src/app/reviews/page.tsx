        ...
        <div className="glass rounded-3xl p-10 border border-amber-500/20 hover-glow-amber transition-all mb-12 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center flex-shrink-0 shadow-xl shadow-amber-500/30">
              <span className="font-playfair text-white font-bold text-3xl">{testimonials[2].avatar}</span>
            </div>
            <div>
              <div className="flex mb-3">{[...Array(5)].map((_,i)=><Star key={i} className="w-5 h-5 text-amber-400 fill-amber-400"/>)}</div>
              <p className="font-playfair text-xl text-white italic leading-relaxed mb-4">&ldquo;{testimonials[2].text}&rdquo;</p>
              <div className="flex items-center gap-3">
                <p className="font-space font-bold text-white">{testimonials[2].name}</p>
                <span className="text-slate-600">—</span>
                <p className="font-outfit text-slate-400 text-sm">{testimonials[2].location}</p>
          ...
          {testimonials.map((r,i)=>(
            <div key={i} className="glass rounded-2xl p-6 border border-slate-800 hover-lift hover-glow-amber transition-all group cursor-default">
              <div className="flex justify-between items-start mb-3">
                <span className="px-3 py-1 rounded-full bg-slate-800 text-xs font-outfit text-slate-400 border border-slate-700">{r.product}</span>
              ...
              <p className="font-outfit text-sm text-slate-300 italic leading-relaxed mb-4">&ldquo;{r.text}&rdquo;</p>
              <div className="flex items-center gap-3 pt-3 border-t border-slate-800">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center flex-shrink-0">
                  <span className="font-playfair text-white font-bold text-sm">{r.avatar}</span>
                </div>
                <div>
                  <p className="font-space text-sm font-semibold text-white">{r.name}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {products.map(p=>(
            <div key={p.id} className="glass rounded-3xl p-8 border border-slate-800 text-center hover-lift hover-glow-amber transition-all">
              ...
              <h3 className="font-playfair text-lg font-bold text-white mb-2">{p.shortName}</h3>
